<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/skeleton/2.0.4/skeleton.css">
    <link rel="stylesheet" href="style.css">
    <title>Complaint Management Portal</title>
</head>
<body>
<div class="container">
        <div class="row">
            <div class="two columns">
                    <img class="mur-logo" src="data:image/png;base64," alt="Murdoch-logo">
            </div>
            <div class="six columns">
                <h2>Complaints Management Portal</h2>
            </div>
            <div class="four columns">
                <h4><a href="#">GUEST</a> | <a href="#">SIGN IN</a></h4>
            </div>
        </div>
    
        <div class="border-markup">
            <div class="row">
                <div class="four columns">
                    <a href="#"> <h5>Complaints Portal Home</h5> </a>
                </div>
                <div class="four columns">
                    <a href="#"> <h5>Create a New Ticket</h5> </a>
                </div>
                <div class="four columns">
                    <a href="#"> <h5>Track Existing Complaint</h5> </a>
                </div>
            </div>
        </div>

        <div class="main-body">
            <div class="row">
                <div class="eight columns">
                    <h4>Welcome to the Complaint Management Portal</h4>
                    <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Ex, quam porro error repellendus sapiente sunt odit repellat facilis, nostrum atque magni. Delectus, eligendi.</p>
                    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dignissimos dolor aliquid officia nam labore culpa. Quia, magni maxime atque sed eos error dolore beatae culpa hic optio necessitatibus sint voluptate. Voluptas quidem, aperiam omnis veniam quis quas deleniti. Libero quasi sunt eaque architecto temporibus optio velit laboriosam, laudantium at!</p>
                </div>
                <div class="four columns">
                    <button class="button-primary disclose">Create A new Ticket</button>
                    <button class="green-button disclose">Track existed Ticket</button>
                </div>
            </div>
        </div>
        
        <hr>
        <div class="footer">
            <p>All copy rights reserved &copy 2019.</p>
        </div>
    </div>
</body>
</html>